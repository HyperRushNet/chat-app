...
export function startChatApp(customConfig = {}) {
    ...
    window.prepareMyAccount = async () => {
        ...
        // Update Lobby Header Avatar
        const lobbyImg = $('lobby-avatar-img');
        const lobbyInit = $('lobby-avatar-initial');
        if(avatar) {
            lobbyImg.src = avatar;
            lobbyImg.style.display = 'block';
            lobbyInit.style.display = 'none';
        } else {
            lobbyImg.style.display = 'none';
            lobbyInit.style.display = 'flex';
            lobbyInit.innerText = (state.user.user_metadata?.full_name || "U").charAt(0);
        }
    };
    ...
    window.handleLogout = async (e) => {
        ...
        state.uptimeInterval = null; state.sessionStartTime = null;
    ...
    window.joinInvite = async () => {
        if(state.inviteId) window.joinAttempt(state.inviteId);
    };
    ...
    const init = async () => {
        ...
        if(inviteId) {
            ...
            if(room) {
                 ...
                 const iconDiv = $('invite-icon');
                 if(room.avatar_url) {
                     iconDiv.innerHTML = `<img src="${room.avatar_url}" style="width:100%;height:100%;object-fit:cover">`;
                 } else {
                     iconDiv.innerText = room.name.charAt(0);
                 }
                 
                 const membersDiv = $('invite-members');
                 membersDiv.innerHTML = '';
                 
                 // Try to show members if not public
                     ...
                     if(profiles && profiles.length > 0) {
                         profiles.forEach(p => {
                             ...
                             <div style="display:flex;flex-direction:column;align-items:center;width:60px">
                                 <div class="chat-avatar" style="width:32px;height:32px;font-size:12px">${p.avatar_url ? `<img src="${p.avatar_url}">` : (p.full_name||'?').charAt(0)}</div>
                                 <span style="font-size:10px;margin-top:4px;color:var(--text-mute);max-width:60px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(p.full_name)}</span>
                             </div>
                             `;
                         });
                     } else {
                         membersDiv.innerHTML = `<div style="color:var(--text-mute);font-size:13px">Private Group</div>`;
                     }
                 } else {
                     membersDiv.innerHTML = `<div style="color:var(--text-mute);font-size:13px">Public Group</div>`;
                 }
                 
                 window.nav('scr-invite');
                 window.setLoading(false);
                 monitorConnection();
                 return; // Stop init here, waiting for user to click Join
            } else {
                window.toast("Invalid invite link");
            }
        }
