...
export function initHRNchat(customConfig = {}) {
    ...
    const monitorConnection = () => {
        const onlineHandler = () => {
            if (state.isOfflineMode) {
                // AANGEPAST: Probeer automatisch online te gaan als internet terugkomt
                window.goOnline();
            } else {
                setConnectionVisuals('connecting');
                if (state.currentRoomId) attemptHardReconnect();
    ...
    state.authListener = db.auth.onAuthStateChange(async (ev, ses) => {
