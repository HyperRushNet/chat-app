...
export function initHRNchat(customConfig = {}) {
		...
		if (useNativeCrypto) {
			...
			if (keyStr) {
				try {
					...
					state.localKey = await crypto.subtle.importKey('jwk', keyData, {
						name: 'AES-GCM'
			...
			state.localKey = await crypto.subtle.generateKey({
				name: 'AES-GCM',
				length: 256
			}, true, ['encrypt', 'decrypt']);
			...
			try {
				...
				if (!error && data) {
					...
					if (msgEl) {
						...
						let prevMsg = null;
						if (prevEl && prevEl.classList.contains('msg')) prevMsg = {
	...
	window.handleLogin = async (e) => {
		...
		if (!state.isOfflineMode) {
			try {
				...
				const {
					error
				} = await promiseTimeout(10000, authPromise);
				if (!error) {
					const {
						data: {
							user
	...
	const startVTimer = () => {
		...
		const tick = () => {
			if (left < 0) {
				...
				window.toast("Code expired.");
				window.nav('scr-register');
				return;
			}
			const m = Math.floor(left / 60);
			const s = (left % 60).toString().padStart(2, '0');
			$('v-timer').innerText = `${m}:${s}`;
			left--;
		};
		tick();
		state.vTimer = setInterval(tick, 1000);
	};
	window.handleVerify = async (e) => {
		...
		const code = $('v-code').value,
			tempStr = sessionStorage.getItem('temp_reg');
		if (!tempStr) {
			window.toast("Session expired. Please restart.");
			state.processingAction = false;
			window.nav('scr-register');
			return;
		}
	...
	const finishReg = async (temp) => {
		try {
			...
			if (data.session && data.user) {
				...
				window.toast("Account created! Please log in.");
				window.nav('scr-login');
				window.setLoading(false);
			}
		} catch (e) {
	...
	window.handleLogout = async (e) => {
		...
		localStorage.removeItem('hrn_mk');
		localStorage.removeItem('hrn_custom_key');
		state.user = null;
		setAppMode(false);
		state.isCapacityBlocked = false;
