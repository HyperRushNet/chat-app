...
export function initHRNchat(customConfig = {}) {
    ...
    const setupChatChannel = (id) => {
        ...
        state.chatChannel.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `room_id=eq.${id}` }, async (payload) => {
            ...
            else if (status === 'CLOSED' || status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') { if (state.connectionTimeoutTimer) { clearTimeout(state.connectionTimeoutTimer); state.connectionTimeoutTimer = null; } if (!state.isOfflineMode && !state.isCapacityBlocked) { state.isChatChannelReady = false; if (!state.isReconnecting) { state.isReconnecting = true; setConnectionVisuals('connecting'); state.reconnectTimer = setTimeout(attemptHardReconnect, 1000); } } }
        });
    };
    ...
    const initRoomPresence = async (roomId) => {
        ...
        .subscribe(async (status, err) => {
            ...
            else if (status === 'CLOSED' || status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') { state.isPresenceSubscribed = false; if (!state.isOfflineMode && !state.isCapacityBlocked) { state.isReconnecting = true; setConnectionVisuals('connecting'); state.reconnectTimer = setTimeout(attemptHardReconnect, 1000); } }
        });
    };
    ...
    window.openRoomInfo = async () => {
        ...
        const delBtn = $('info-delete-btn'); const creatorRow = $('info-creator-row'); delBtn.style.display = 'none'; delBtn.innerText = "Delete Chat"; delBtn.classList.remove('active'); if (state.deleteConfirmTimeout) clearTimeout(state.deleteConfirmTimeout);
        ...
        if (room.is_direct) {
            ...
            if (otherId) { const profile = await getProfile(otherId); if (profile) { $('info-name').innerText = profile.full_name; const avEl = $('info-avatar'); if (profile.cached_avatar || profile.avatar_url) avEl.innerHTML = `<img src="${profile.cached_avatar || profile.avatar_url}">`; else avEl.innerText = (profile.full_name || 'U').charAt(0); } else { $('info-name').innerText = 'User'; } }
            delBtn.style.display = 'flex';
        } else {
            $('info-type').innerText = "Group Chat"; $('info-name').innerText = room.name;
            const avEl = $('info-avatar'); if (room.avatar_url) avEl.innerHTML = `<img src="${room.avatar_url}">`; else avEl.innerText = room.name.charAt(0);
            creatorRow.style.display = 'flex';
            if (room.created_by) { let profile = state.profileCache[room.created_by]; if (!profile) { const { data } = await db.from('profiles').select('full_name').eq('id', room.created_by).single(); profile = data; if (data) state.profileCache[room.created_by] = data; } $('info-creator').innerText = profile?.full_name || 'Unknown'; } else $('info-creator').innerText = 'Unknown';
    ...
    window.openRoomSettings = async () => {
        window.closeOverlay(); if (!state.currentRoomId || !state.currentRoomData || state.currentRoomData.created_by !== state.user.id) return window.toast("Not owner");
        window.setLoading(true, "Loading..."); const room = state.currentRoomData; state.currentStep.edit = 1; updateStepUI('edit');
    ...
    window.saveRoomSettings = async (e) => {
        ...
        state.currentRoomData = updatedRoom; $('chat-title').innerText = updatedRoom.name; window.toast("Saved"); window.closeOverlay(); state.processingAction = false; window.setLoading(false);
    };
    ...
    window.openVault = async (id, n, rawPassword, roomSalt, cachedData = null) => {
        ...
        if (!state.isOfflineMode) {
            ...
            } catch (e) { console.warn("Network fetch failed, using local", e); }
        }
        ...
        if (isDirect) {
            ...
            if (otherUserId) {
                ...
                if (profile) { displayTitle = profile.full_name; displayAvatar = profile.cached_avatar || profile.avatar_url; }
            }
        }
        ...
        if (!state.isOfflineMode) {
            ...
            } catch (e) { console.warn("Failed to fetch online messages", e); }
        }
    ...
    window.sendMsg = async (e) => {
        ...
        } catch (err) { window.toast("Encryption or Send failed"); }
    ...
    window.handleLogin = async (e) => {
        if (!e || !e.isTrusted) return; if (state.processingAction) return;
        state.processingAction = true;
        ...
        if (state.isOfflineMode) {
            ...
            if (knownUser && knownUser.metadata) {
                ...
                if (knownUser.pass_hash && knownUser.pass_hash === hashInput) {
                    ...
                    window.nav('scr-lobby'); window.loadRooms(); window.setLoading(false); state.processingAction = false; window.toast("Offline Login Successful"); return;
                }
            }
        ...
        if (error) {
            ...
            const { data: { user } } = await db.auth.getUser();
            state.user = user; setAppMode(false);
            const hashInput = await sha256(p + em);
    ...
    window.handleRegister = async (e) => {
        ...
        } catch (err) { window.toast("API Fallback: Proceeding without code (Dev Mode)"); sessionStorage.setItem('temp_reg', JSON.stringify({ n, em, p, avatar: avatarUrl })); window.nav('scr-verify'); startVTimer(); window.setLoading(false); }
        state.processingAction = false;
    };
    ...
    window.handleVerify = async (e) => {
        ...
        try {
            ...
            if (r.status === 429) { window.toast("Rate limited"); state.processingAction = false; window.setLoading(false); return; }
            const j = await r.json();
            if (j.message === "Verified") await finishReg(temp); else { window.toast(j.message || "Wrong code"); window.setLoading(false); }
        } catch (err) { window.toast("API Fallback: Auto-verifying (Dev Mode)"); await finishReg(temp); }
        state.processingAction = false;
    };
    const finishReg = async (temp) => { const { error } = await db.auth.signUp({ email: temp.em, password: temp.p, options: { data: { full_name: temp.n, avatar_url: temp.avatar } } }); if (error) { window.toast(error.message); window.setLoading(false); } else { localStorage.setItem('hrn_auth_email', temp.em); localStorage.setItem('hrn_auth_pass', temp.p); window.nav('scr-lobby'); window.loadRooms(); window.setLoading(false); } };
    window.handleCreate = async (e) => {
        ...
        } else { n = $('c-name').value.trim(); avatarUrl = $('c-avatar').value.trim() || null; rawPass = $('c-pass').value; isVisible = $('c-visible').checked; if (!n) { window.toast("Name required"); state.processingAction = false; return; } }
        let allowedUsers = ['*'];
    ...
    window.handleLogout = async (e) => { if (!e || !e.isTrusted) return; window.setLoading(true, "Leaving..."); await cleanupChannels(); localStorage.removeItem('hrn_auth_email'); localStorage.removeItem('hrn_auth_pass'); state.user = null; setAppMode(false); state.isCapacityBlocked = false; await db.auth.signOut(); window.nav('scr-start'); window.setLoading(false); };
